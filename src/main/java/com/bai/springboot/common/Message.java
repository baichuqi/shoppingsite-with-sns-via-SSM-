package com.bai.springboot.common;
...
import com.bai.springboot.service.impl.MessageBeanServiceImpl;
...
public class Message {
	 ...
	 private static Logger LOG = LoggerFactory.getLogger(Message.class);
	    ...
	    public void send(String message, Session filterSession) {
	        ...
	        text.setUserName(filterSession.getId());//NickPool.get(filterSession.getId())
	        //text.setSend();
	        LOG.info("now username is"+text.getUserName()+" \n");
	    ...
	    public void sendto(String message, Session filterSession) {
	    	
	    	//从昵称池拿出用户昵称
	        Text text = JsonUtil.getBean(message, Text.class);
	        text.setUserName(filterSession.getId());//NickPool.get(filterSession.getId())
	        LOG.info("now username is"+text.getUserName()+"sent to"+text.getSend()+" \n");
	        //判断是否是需要执行的脚本，是则return，由代理处理脚本
	        if(CMD.isCMD(text.getMessage())) {
	            return;
	        }
	        LOG.info("send to no.1 user ");
	        //send to no.1 user
	        Set<String> keys = UserPool.getUserPool().keySet();
	        for(String key : keys) {
	            Session session = (Session) UserPool.get(key);
